...
export default function App() {
  ...
  const [role, setRole] = useState('user'); // Сразу ставим 'user', чтобы не ломать загрузку
  const [loading, setLoading] = useState(true);

  // Исправленная функция: никогда не заблокирует экран загрузки
  const fetchUserRole = async (userId) => {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('role')
        .eq('id', userId); // Убрали .single(), чтобы не вызывать ошибку пустого ответа

      if (data && data.length > 0 && !error) {
        setRole(data[0].role); // Берем роль ('admin' или 'user')
      } else {
        setRole('user'); // Если профиля нет в базе, ты просто обычный клиент
      }
    } catch (err) {
